...
\begin{quote}
 ...
 \begin{itemize}
  ...
  \item[\inline{speclines}] Opens the spectral lines browser window to retrieve 
  spectral lines from  VO (SLAP) and VAMDC services
 ...
 \item \subsubmenuitem{Options}{Line identifiers}{Line Browser}
  ...
  spectral lines using the VO and VAMDC.
...
\SPLAT\ supports server-side manipulation through \textbf{Datalink/SODA} (Server-side Operations for Data Access). After a SSAP Query is sent, \SPLAT\ checks the services' responses  for  \textbf{Datalink/SODA} parameters, and marks them with a symbol (\inline{cutter}). If at least one supporting service has been found, a \textbf{Datalink} button will appear below the query results. By clicking on it, the button will be activated and all results tabs will be deactivated except the results from servers that support \textbf{Datalink/SODA}. When you choose one of the active services, a window will appear, where the respective parameters for sever-side manipulation can be entered. After editing the parameters set them by clicking on the \textbf{set parameters} button. While  the \textbf{Datalink} button is activated and the parameters are set, the spectra you select, display or download will be manipulated using the chosen parameters. To deactivate it and retrieve the unmodified original spectra just click on the \textbf{Datalink} button again to deactivate it.

\mainfigure{datalinkparams}

\subheading{Implementation status}

SSAP, DataLink, and SODA are  being developed by the IVOA
(\htmladdnormallinkfoot{International Virtual Observatory Alliance}
{http://www.ivoa.net/}).

Spectra returned from SSAP servers are mainly FITS and VOTables. SPLAT handles
also VOTables using DataLink and SODA standards. 
Besides that, SPLAT tries to recognise and display Timeseries. This function is still a prototype and
will be further developed following the development of the time domain standards by IVOA.

%Currently spectra returned from SSAP servers may be in basic formats,
%that is simple FITS and VOTables (partially some SED data model serialisation).

%However, 
The \SPLAT\ data model includes errors as well as coordinates and
data values.
...
functionality. One difference is the "data product" option, where either spectrum' or 'timeseries' can be chosen.
...
The selection of the services to be queried is similar to the one SSAP and Obscore browsers.
In this case, there are two tabs, one for SLAP services, and other for VAMDC services.

The {Search} Button sends the query to the selected services. The results will appear on the
...
The {Remove lines} button removes all lines from the plot.
...
Spectra retrieved by the VO often do not have an extension, and their types
can also be discovered by checking the mime type returned.
